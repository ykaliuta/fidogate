...
{
    sprintf(buffer, "%s/ftntick -x %s/ftntickpost -I %s %s", libexecdir,
            libexecdir, inbound, verbose_flag);
...
void run_toss(Runtoss * a)
{
    ...
    if (statfs(a->inbound, &drive) && (minfree < drive.f_bavail * drive.f_bsize)
        &&
#else
    if (statvfs(a->inbound, &drive)
        && (minfree < drive.f_bavail * drive.f_bsize) &&
#endif
        (drive.f_ffree > 100)) {
                ...
                if (pkt_flag) {
                    if (b_flag)
                        if ((ret = system_run(b_flag)) != 0) {
                            fglog("$WARNING: before toss script return %d",
                                  ret);
                            unlock_program(PROGRAM);
                            exit_free();
                            exit(2);
