...
Создатель Andrey Slusar 2:467/126, santinel<собака>ua.fm.
...
3.Установка и начальная настройка Exim4.
4.Установка и начальная настройка Postfix и Procmail.
5.Установка и настройка ньюссервера leafnode.
6.Установка и настройка ньюссервера inn.
7.Компиляция и установка FIDOGATE.
8.Окончательная настройка совместной работы с FIDOGATE MTA и ньюссервера.
9.Примечания и часто задаваемые вопросы по работе.
10.Работа FIDOGATE5.
...
------------------------------------------------------------------------------
2.Правим свой хост и домен для нормального хождения почты и ньюсов.
...
------------------------------------------------------------------------------
4.Устанавливаем Postfix и Procmail(не обязательно) так, чтобы ходила локальная 
  почта:
...
-------------------------------------------------------------------------------
7. Устанавливаем FIDOGATE5:

7.1.Берем исходники последнего FIDOGATE5 или с cvs(все в одну строчку):
  ===
...
7.2.Можно просто взять у меня на сайте http://node126.narod.ru файл 
  fidogate-5.0.0-beta7.src.rpm и перекомпилировать:
  ...
  или воспользоватьс готовым бинарным пакетом fidogate-5.0.0-beta7.rpm:
...
7.3. Для freebsd можно взять пакадж с моего сайта:
...
9.Ответы на часто задаваемые вопросы:
  ...
  A(AS):Необходимо создать корректный history inn'a:
  ...
  A(AS):Для повышения надежности работы ньюссервера можно вместо storage метода 
  ...
  A6:Необходимо наконец прочитать INSTALL в пакете leafnode и прописать правильно
     доступ в hosts.allow, hosts.deny.
