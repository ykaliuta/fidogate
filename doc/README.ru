...
`TIC_HISTORY'(Deprecated, moved to runtime NoTicHistory)
...
эхоконференциям и групповых настроек для конференций.
 ...
 -C def:in:out:def_rfc настройки чарсетов для эхообласти.
...
# Подерживать PING/TRACE. Генерировать ответы при получении нетмейла
# (в том числе транзитного) на имя PING (регистронезависимо)
# См. http://ftsc.org/docs/fts-5001.006 (5.10. Robot flags)
SupportPing
...
# Добавить "tic" контекст в passwd (если filefix и tic пароли отличаются)
#EnableTickPassword

# Отключить дуполовку tic
#NoTicHistory
